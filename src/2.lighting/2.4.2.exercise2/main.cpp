...
{
    // switch gui and WSAD/Mouse
    static int space = GLFW_RELEASE;
    auto spaceNow = glfwGetKey(window, GLFW_KEY_SPACE);
    if (space == GLFW_PRESS && spaceNow == GLFW_RELEASE)
    {
        capturedByImGui = !capturedByImGui;
        if (capturedByImGui)
...
int main(int argc, char const *argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            ImGui::Begin("Options");
            ImGui::Text("Press <space> to switch to FPS mode");
            ImGui::Checkbox("Moving Lamp", &movingLamp);
            ImGui::ColorEdit3("Object Color", glm::value_ptr(objectColor));
            ImGui::ColorEdit3("Light Color", glm::value_ptr(lightColor));
            ImGui::SliderFloat("Ambient Strength", &ambientStrength, 0, 1.0f, "%.02f", 1.0f);
            ImGui::SliderFloat("Specular Strength", &specularStrength, 0, 1.0f, "%.02f", 1.0f);
        ...
        if (movingLamp)
            lightPos = glm::vec3(sin(currentFrame), cos(currentFrame), sin(currentFrame));
